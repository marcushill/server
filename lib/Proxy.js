...
Proxy.prototype._handle_socket = async function(socket) {
    const self = this;
    ...
    if (self.activeSockets === 0) {
      await self.redx.addConnection(self.id);
    }
...
Proxy.prototype._cleanup = async function() {
    const self = this;
    ...
    await self.redx.removeConnection(self.id);
